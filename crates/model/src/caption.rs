...
/// The asterisk family working tools cycle through, plus the braille
/// spinner range.
...
/// A quiet leading marker the same tools leave when idle.
...
mod tests {
    ...
    fn spinner_glyphs_mean_working() {
        for raw in ["✻ Finish M2 tasks", "✳ Finish M2 tasks", "⠋ building"] {
    ...
    #[test]
    fn idle_dot_is_stripped_but_not_working() {
        let c = decode_title("· Review Slack thread");
        assert!(!c.working);
